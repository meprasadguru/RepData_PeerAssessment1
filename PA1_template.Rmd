...
```{r fig.height=4}
hist(total_steps_per_day$steps,xlab="Total Steps per day",main="Histogram of total number of steps taken each day",col="green")
```
...
Average_steps_per_interval$interval_formated <- format(strptime(stringformat,format="%H%M"),format="%H:%M")
plot(Average_steps_per_interval$steps,xaxt="n",type='l',col='red',xlab="Time of Day",ylab="Average steps",main="Avaerage Number of steps taken per inerval acrros all days")
axis(1,at=seq(1,288,by=12),labels=Average_steps_per_interval$interval_formated[seq(1,288,by=12)])
```
...
```{r}
max_steps_interval    <- Average_steps_per_interval[which(Average_steps_per_interval$steps == max(Average_steps_per_interval$steps), arr.ind = TRUE),]
```
**`r max_steps_interval[,3]`** interval on an average across all the days in the dataset, contains the maximum number of steps **`r max_steps_interval[,2]`**.
...
```
The total number of missing values in the dataset *activity* is `r total_missing_values` .
...
x <- c(x,c)}
new_data$steps         <- replace(new_data$steps,missing_index,Average_steps_per_interval[x,2])
...
new_data$day            <- factor((weekdays(new_data$date) %in% wdays),levels=c(FALSE,TRUE),labels=c('weekend','weekday'))
Average_steps_interval  <- aggregate(steps~interval,new_data,FUN="mean")

xyplot(steps~interval | day,data=new_data,layout=c(1,2),type='l',main="Avaerage Number of steps per inerval acrros weekdays and weekends")
```
